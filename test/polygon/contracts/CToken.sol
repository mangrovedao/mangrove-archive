...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
   ...
   * @return The borrow interest rate per block, scaled by 1e18
   */
  function estimateBorrowRatePerBlockAfterChange(uint change, bool repay)
    external
    view
    returns (uint)
  {
   ...
   * @return The supply interest rate per block, scaled by 1e18
   */
  function estimateSupplyRatePerBlockAfterChange(uint change, bool repay)
    external
    view
    returns (uint)
  {
  ...
  function exchangeRateStoredInternal() internal view returns (uint) {
    ...
    if (_totalSupply == 0) {
      ...
      uint exchangeRate =
        div_(cashPlusBorrowsMinusReserves, Exp({mantissa: _totalSupply}));
      return exchangeRate;
    }
  }
   ...
   */
  function mintInternal(uint mintAmount, bool isNative)
    internal
    nonReentrant
    returns (uint, uint)
  {
   ...
   */
  function redeemInternal(uint redeemTokens, bool isNative)
    internal
    nonReentrant
    returns (uint)
  {
   ...
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function redeemUnderlyingInternal(uint redeemAmount, bool isNative)
    internal
    nonReentrant
    returns (uint)
  {
   ...
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function borrowInternal(uint borrowAmount, bool isNative)
    internal
    nonReentrant
    returns (uint)
  {
   ...
   */
  function repayBorrowInternal(uint repayAmount, bool isNative)
    internal
    nonReentrant
    returns (uint, uint)
  {
  ...
  function liquidateBorrowFresh(
    ...
    (uint repayBorrowError, uint actualRepayAmount) =
   ...
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function _addReservesInternal(uint addAmount, bool isNative)
    internal
    nonReentrant
    returns (uint)
  {
   ...
   */
  function _addReservesFresh(uint addAmount, bool isNative)
    internal
    returns (uint, uint)
  {
   ...
   */
  function getCTokenBalanceInternal(address account)
    internal
    view
    returns (uint);
