...
abstract contract Mangrove {
  ...
  */
    ...
    returns (MC.Config memory ret)
  {
    ...
    ret.local = MC.Local({
      ...
      tickrefine: $$(local_tickrefine("_local")) > 0,
      tickpower: $$(local_tickpower("_local"))
    });
  }
  ...
  */
  ...
  /* <a id="Mangrove/definition/writeOffer"></a> */
  ...
  function writeOffer(OfferPack memory ofp, bool update) internal {
    ...
    To make the price tick configurable we need a large band of available ticks. Suppose we want a precision of exactly 1 cent of the base currency on the pairs USDC/DAI and DAI/USDC. USDC has 6 decimals of precision while DAI has 18. 
    ...
    */
      ...
      uint ticksize = 10**$$(local_tickpower("ofp.local"));

      /* In the following, `gives` is $g$, `wants` is $w$, `ticksize` is $s$. */
      //+clear+
      /* If the tick is configured for precision finer than 1 wei, we do the following:
      ...
      if ($$(local_tickrefine("ofp.local")) > 0) {
         ...
         * Compute $p = \left\lceil\dfrac{w}{g\times s}\right\rceil$.
         * Let $w' = p \times s \times g$.
         * The new price $\frac{w'}{g}$ is at the desired precision.
         */
  ...
  function setTick(
    ...
    locals[base][quote] = $$(
      set_local(
        "locals[base][quote]",
        [["tickrefine", "_tickrefine"], ["tickpower", "tickpower"]]
      )
    );
    emit MgvEvents.SetTick(base, quote, tickrefine, tickpower);
  }
