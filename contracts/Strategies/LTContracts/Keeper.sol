...
contract Keeper is AccessControlled, CarefulMath {
    ...
    function collect(address base, address quote, uint[] memory offers) external {
    ...
    function touchAndCollect(address base, address quote, uint offerId, uint gives) external {
        (bool success,,) = MGV.snipe(base,quote, offerId,0,gives,uint(MAXUINT24),false);
        require(!success, "Collect failed");
    }

    function transfer(address erc, address recipient, uint amount) external onlyAdmin returns (bool) {
