...
contract Defensive is MangroveOffer, Exponential, OpenOraclePriceData {
  ...
  address constant coinbaseReporter = 0xfCEAdAFab14d46e20144F48824d0C09B1a03F2BC;

  constructor(address _priceFeed, address _trustedSource, address payable _MGV) 
  MangroveOffer(_MGV) {
    priceFeed = OpenOraclePriceData(_priceFeed);
    trustedSource = _trustedSource;
  }

  function __lastLook__(MgvCommon.SingleOrder calldata order)
    internal
    override
    returns (uint)
  {
    ...
    uint usd_wanted = mul_( //amount of base tokens required by taker (in ~USD, 6 decimals)
      order.wants, 
      uint(priceFeed.getPrice(trustedSource, base.symbol())) //could be checking age of the time stamp of data
    );
    uint usd_given = mul_( //amount of quote tokens given by taker (in ~USD, 6 decimals)
      order.gives, 
