...
async function logLenderStatus(contract, lenderName, tokens) {
  async function getAaveBorrowBalance(symbol) {
    ...
    const reserveData = await pool.getReserveData(getContract(symbol).address);
    ...
    const stableDebt = new hre.ethers.Contract(stableDebtTokenAddress, env.abis.stableDebtToken, hre.ethers.provider);
    const variableDebt = new hre.ethers.Contract(variableDebtTokenAddress, env.abis.variableDebtToken, hre.ethers.provider);
    console.log(stableDebt);
  ...
  if (lenderName == "compound") {
    baseUnit = "USD";
    [, borrowPower] = await getContract("COMP").getAccountLiquidity(contract.address);
  ...
  console.log();
  console.log(
    `**** Account borrow power (${baseUnit}): \x1b[35m`,
  ...
  for (const symbol of tokens) {
    switch (lenderName) {
      case "compound" : {
        ...
        const balance = await getContract(symbol).balanceOf(contract.address);
        logPosition(
          symbol,
          formatToken(redeemable, symbol),
          formatToken(borrowBalance, symbol),
          formatToken(balance, symbol)
        );
        break;
      }
      case "aave" : {
        const token = getContract("symbol");
        const [stableBorrowBalance,variableBorrowBalance] = await getAaveBorrowBalance(symbol);
        const borrowBalance = formatToken(stableBorrowBalance, symbol)+"/"+formatToken(variableBorrowBalance, symbol);
