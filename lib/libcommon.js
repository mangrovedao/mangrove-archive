...
async function logLenderStatus(contract, lenderName, tokens) {
  ...
  switch (lenderName) {
    case "compound" :
      lender = getContract("COMP");
      overlying0 = getContract("CDAI");
      overlying1 = getContract("CWETH");
      [,borrowPower] = await lender.getAccountLiquidity(contract.address);
      baseUnit = "USD";
      break;
    case "aave" :
      lender = net.aave.lendingPool;
      overlying0 = getContract("ADAI");
      overlying1 = getContract("AWETH");
      borrowPower = await lender.getUserAccoundData(contract.address).availableBorrowsETH;
      baseUnit = "ETH";
      break;
    default:
