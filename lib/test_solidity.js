...
module.exports = (
  ...
  const debug = require("debug")("buidler:test-solidity");
  const ethers = bre.ethers; // Local ethers.js shortcut
  const deploy = bre.deploy;
  ...
  const getArtifact = (name) => {
    const c = artifacts.find((c) => c.contractName == name);
    if (!c) {
  ...
  const tryDisplayLog = async (rawLog) => {
    ...
    for (const artifact of artifacts) {
      ...
      console.log(`Event ${log.signature} (in ${artifact.contractName})`);
      ...
      for (const file in artifact.linkReferences) {
        for (const libName in artifact.linkReferences[file]) {
          if (!deployedLibraries[libName]) {
            ...
            const lib = await bre.deployments.deploy(libName, opts);
  ...
  const deployWithPres = async (currentName) => {
    ...
    const deployed = await bre.deployments.deploy(currentName, opts);
  ...
  const testableContracts = artifacts.filter((c) =>
    artifacts.some((c2) => c2.contractName == schema.toTest(c.contractName))
  );
  ...
  const testContracts =
    argTestContractNames.length == 0
      ? testableContracts
      : artifacts.filter((c) => argTestContractNames.includes(c.contractName));
    ...
    for (const contract of testContracts) {
      ...
      for (const fn in testContract.interface.functions) {
        if (
          ...
          suite.beforeAll(
            `Before testing ${contract.contractName}.${fn}`,
            async function () {
              debug("Running beforeAll named %s", fn);
        ...
        if (schema.isTestFailFunction(fn)) {
          ...
          const test = new Mocha.Test(fn, async () => {
            ...
            for (const rawLog of receipt.logs) {
              let log;
              try {
                log = testContract.interface.parseLog(rawLog);
              } catch (e) {}
              if (log) {
                if (!log.args.success) {
