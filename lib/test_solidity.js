...
module.exports = (
  ...
  const tryDisplayLog = (log, rawLog, originator) => {
    if (!log) {
      ...
      console.log(` issued by ${originator.contractName} (address ${address})`);
      ...
      for (const [name, { value, type }] of normalizedArray) {
        console.log(`  ${name.padEnd(padLength, " ")}: ${value} (${type})`);
      }
      console.log("");
    }
  };
          ...
          if (log && schema.events[log.name]) {
            ...
            while (true) {
              if (unknownEvents.length === 0) {
                ...
                tryDisplayLog(log, rawLog, testContractObj);

                assert.fail("Missed expected events. See above.");
                break;
