...
module.exports = (
  ...
  extendConfig((config, userConfig) => {
    config.logFormatters = userConfig.logFormatters || {};
  });
    ...
    for (const testContractObj of testContracts) {
      ...
      const processLogs = (receipt) => {
        ...
        for (const rawLog of receipt.logs) {
          ...
          if (log && config.logFormatters[log.name] && details) {
            processed = true;
            config.logFormatters[log.name](log, rawLog, originator, formatArg);
          }
