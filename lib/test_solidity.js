    ...
    for (const testContractObj of testContracts) {
      ...
      const processLogs = (receipt) => {
        ...
        for (const rawLog of receipt.logs) {
          ...
          if (log && config.logFormatters[log.name]) {
            processed = true;
          ...
          if (rawLog.address === testContract.address) {
            ...
            if (!processed && expectations.list.length > 0) {
        ...
        if (expectations.list.length > 0) {
          console.log(
            "Missing some expected events, use --show-events and --show-test-events to see all events received."
          );
          console.log("Missed events:");
          expectations.list.forEach(({ address, raw, log }) => {
            let parsedAddress = getRegister(address) || address;
            console.log(`Expected from ${parsedAddress}:`);

            tryDisplayLog(log, raw, testContractObj);
          });
