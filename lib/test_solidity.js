...
module.exports = (
  ...
  const mocha = new Mocha(config.has("mocha") ? config.get("mocha") : {}); // Instantiate to generate tests
  ...
  const formatArg = (arg, type /*optional*/) => {
    if (Array.isArray(arg)) {
      const newtype = type.slice ? type.slice(0, -2) : undefined;
      return arg.map((a) => formatArg(a, newtype));
    }
  ...
  let logFormatters = {};
  try {
    logFormatters = require("./log_formatters.js")(hre, formatArg);
  } catch (e) {}
  ...
  //Returns an object {name: {value,type},...}
  ...
  //return {name, value, type: input.type};

  const tryDisplayLog = (log, rawLog, originator) => {
    console.log(log, rawLog, originator);
  };

  const logToString = (log, rawLog, originator) => {
    if (!log) {
      return "Could not parse the following raw log:\n" + util.inspect(rawLog);
    } else {
      ...
      return (
        `Event ${log.signature}\n` +
        ` issued by ${originator.contractName} (address ${address})\n` +
        normalizedArray
          .map(
            ([name, { value, type }]) =>
    ...
    for (const testContractObj of testContracts) {
      ...
      const processLogs = (receipt) => {
        ...
        for (const rawLog of receipt.logs) {
          ...
          if ((fromTest && showTestEvents) || (!fromTest && showEvents)) {
          ...
          if (log && schema.events[log.name]) {
            ...
            const match = (rawLog, rawExpect) => {
              return (
                rawLog.address === expectedAddress &&
                rawLog.topics.every((t, i) => rawExpect.topics[i] === t) &&
                rawLog.data === rawExpect.data
              );
            };
            while (true) {
              if (unknownEvents.length === 0) {
                ...
                const parsedAddress =
                  getRegister(expectedAddress) || expectedAddress;
                console.log(`Expected from ${parsedAddress}:`);

                const logAsString = logToString(log, rawLog, testContractObj);
          ...
          if (log && logFormatters[log.name] && details) {
            logFormatters[log.name](log, rawLog, originator, formatArg, hre);
          }
        }
      };
      ...
      sortedFunctions = Object.entries(testContract.interface.functions).sort(
        ([fnA], [fnB]) => fnA.localeCompare(fnB, "en", {})
      );
      ...
      for (const [fnName, fnFragment] of sortedFunctions) {
        ...
        if (schema.isTestFailFunction(fnName)) {
          ...
          const fnRootName = schema.toRootName(fnName);
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      argTestContractNames.forEach(async (name) => {
        try {
