...
extendEnvironment((hre) => {
  ...
  hre.env.mainnet = {
    ...
    name : networkName,
    tokens: getConfiguredTokens(mainnetConfig, hre),
    abis: getExtraAbis(mainnetConfig)
  };

  const mangrove = tryGetMangroveEnv(mainnetConfig, hre);
  if (mangrove) {
    hre.env.mainnet.mgv = mangrove;
  }

  const compound = tryGetCompoundEnv(mainnetConfig, hre);
  if (compound) {
    hre.env.mainnet.compound = compound;
  }

  const aave = tryGetAaveEnv(mainnetConfig, hre);
  if (aave) {
...
function getExtraAbis(mainnetConfig) {
  ...
  if (mainnetConfig.has("extraAbis")) {
    abis.stableDebtToken = mainnetConfig.get("extraAbis.stableDebtToken");
    abis.variableDebtToken = mainnetConfig.get("extraAbis.variableDebtToken");
  }
  return abis;
}

function getConfiguredTokens(mainnetConfig, hre) {
  let tokens = {};
...
function tryCreateTokenContract(tokenName, configName, mainnetConfig, hre) {
...
function tryGetCompoundEnv(mainnetConfig, hre) {
...
function tryGetAaveEnv(mainnetConfig, hre) {
  ...
  console.info(
    `Ethereum Aave ABI loaded. LendingPool is at: ${lendingPoolAddress}`
  );
  return aave;
}

function tryGetMangroveEnv(mainnetConfig, hre) {
