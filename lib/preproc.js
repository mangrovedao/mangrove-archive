...
// validate struct: total size is <256 bits, each bitsize is divisible by 4 (since bitmasks work at the nibble granularity level).
const validate = (sname, obj_struct) => {
  const red = (acc, field) => {
    if (field.bits % 4 != 0) {
      throw new Error(
  ...
  const bits = obj_struct.reduce(red, 0);
  if (bits > 256) {
    throw new Error(`bad struct ${sname}, bitsize ${bits} > 256`);
  }
};

exports.structs_with_macros = (obj_structs) => {
  for (const sname in obj_structs) {
    validate(sname, obj_structs[sname]);
  }

  const structs = Object.entries(obj_structs).map(([sname, struct]) => [
    sname,
    struct.map(({ name, bits, type }) => [name, bits, type]),
  ]);
